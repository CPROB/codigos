/*ENTA Team SAT3 flight software code v.8
Includes procedures to get information from all sensors, builds the data string and sends
IMU values int. Includes string number & CRC calculation*/
...
float temp, pressure, altitude, hum, roll, pitch, heading, x, y, z, voltage, real_voltage, 
      lat, lon, sec_lat, sec_lon, dec_lat, dec_lon, prev_dec_lat, prev_dec_lon;
String s_temp, s_pressure, s_altitude, s_hum, s_roll, s_pitch, s_heading, s_x, s_y, s_z, s_dec_lat, s_dec_lon, sat_data, 
       BME280_status, DOF_status, s_volts, s_power_origin, buzz;
...
char o;
const float T0 = 19.3, P0 = 1018,  H0 = 45; // values to be uploaded before each mission + etslog
...
void setup()
{
  ...
  altitude=H0;
  prev_dec_lat=0.000000;
  prev_dec_lon=0.000000;
  GPS.begin(9600);
...
void loop()
{
  ...
  if (millis() - timer > rate) 
  {
    ...
    buzz = (String)set_buzzer; //update buzzer status
    
    //evaluates altitude for buzzer activation
    if (altitude > 150 && altitude <1000) 
      h = 1;
    if ((altitude < 150) && (h == 1)) 
      activateBuzzer(); // calls for procedure if bellow 200 meters
    ...
    sat_data+="%"; //add separator for CRC
    ...
    myFile.print(sat_data); 
    myFile.println(value,HEX);
    myFile.close();
   
    lnumb++; //update line number
...
void getVoltage()
{
  voltage = (((float)analogRead(A0)*5)/1023);
  if (voltage>2)
  {
